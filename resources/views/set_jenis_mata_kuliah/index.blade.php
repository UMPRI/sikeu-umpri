...
<section class="content">
  <div class="container-fluid-title">
    <div class="title-laporan">
      <h3 class="text-white">Pengelompokan Mata Kuliah Yang Ditawarkan</h3>
    </div>
  </div>
  <div class="container">
    <div class="panel panel-default bootstrap-admin-no-table-panel">
      <div class="panel-heading-green">
        <div class="bootstrap-admin-box-title right text-white">
          <b>Pengelompokan Mata Kuliah Yang Ditawarkan</b>
        </div>
      </div>
        <br>
                            ...
                            <table class="table table-striped table-bordered table-hover table-sm table-font-sm">
                              ...
                              foreach ($acd_offered_course as $aoc)
                                { ?>
                                    <tr>
                                        <td>
                                            <input type="checkbox" value="<?php echo $aoc->Course_Id ?>" name="CourseId[]" id="CourseId" class="Offered_Course" />
                                        </td>
                                        <td>
                                          ...
                                          { ?>
                                              <tr>
                                                  ...
                                                  <td>
                                                    {{-- cek data sks --}}
                                                    <?php
                                                    $linksks = "biaya_per_sks?Term_Year_Id=".$Term_Year_Id."&Class_Prog_Id=".$Class_Prog_Id;
                                                    ?>
                                                    <a href="{{ asset($linksks) }}">
                                      ...
                                      foreach ($Course_Cost_Type_Paket as $cctt)
                                          { ?>
                                              <tr>
                                                  ...
                                                  <td>
                                                      {{-- cek keberadaan data packed --}}
                                                      <?php
                                                      $link = "biaya_per_paket/create?Term_Year_Id=".$Term_Year_Id."&Department_Id=".$Department_Id."&Class_Prog_Id=".$Class_Prog_Id."&Course_Name=".$cctt->Course_Name;
                                                      if ($Package_Count != 0) {
                                                        $link = "biaya_per_paket?Term_Year_Id=".$Term_Year_Id."&Department_Id=".$Department_Id."&Class_Prog_Id=".$Class_Prog_Id;
                                                      }
                                                      ?>
                                                      <a href="{{ asset($link) }}">
                                                        {{ $cctt->Course_Name." ".$cctt->Course_Code }}
                                                      </a>
                                                      {{-- cek keberadaan data packed --}}
                                                  </td>
                                              </tr>
  ...
  <script type="text/javascript">
        ...
        $(document).ready(function () {
          // pilih semua
          $('#pilih_semua').click(function(event) {
            if (this.checked) {
              $('.Offered_Course').each(function() {
                this.checked = true;
              });
            }else {
              $('.Offered_Course').each(function() {
