...
<section class="content">
                ...
                <div class="panel panel-default panel-green" >
                    ...
                    <div class="bootstrap-admin-panel-content bootstrap-admin-no-table-panel-content" style="background-color:white;padding:10px;color:#333;">
                        <div class="form-horizontal">
                            <form class="" action="" method="post">
                                <input type="hidden" name="Cost_Scheme_Discount_Id" id="Cost_Scheme_Discount_Id" value="{{ $Cost_Scheme_Discount->Cost_Scheme_Discount_Id }}">
                                <div class="form-group">
                                    <label class = "control-label col-md-12">Kode Diskon</label>
                                    <div class="col-md-12">
                                        <input type="text" name="Discount_Code" class="form-control form-control-sm" placeholder="Masukan Kode Diskon" id="Discount_Code" value="{{ $Cost_Scheme_Discount->Discount_Code }}">
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-md-12">
                                        <input type="text" name="Cost_Scheme_Discount_Name" class="form-control form-control-sm" placeholder="Masukan Nama Diskon" id="Cost_Scheme_Discount_Name" value="{{ $Cost_Scheme_Discount->Cost_Scheme_Discount_Name }}">
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-md-12">
                                        <div class="row">
                                            <div class="col-md-8">
                                                <input type="number" min="1" max="8" name="Term_Total" class="form-control form-control-sm" placeholder="Masukan Masa Berlaku" id="Term_Total" value="{{ $Cost_Scheme_Discount->Term_Total }}">
                    ...
                    <div style="text-align:right">
                        ...
                        <button type="button" id="submit" class="btn btn-success">
                            Update Diskon<span class="glyphicon glyphicon-floppy-save" style="color:white" aria-hidden="true"></span>
                        </button>
                    </div>
                    </div>
                </div>
    ...
    <?php
    $array_dicount_detail = [];
    $ii = 0;
    foreach ($Cost_Scheme_Discount_Detail as $Discount_Detail) {
        $array_dicount_detail[$ii]['Cost_Item_id'] = $Discount_Detail->Cost_Item_Id;
        $array_dicount_detail[$ii]['Cost_Item_Name'] = $Discount_Detail->Cost_Item_Name;
        $array_dicount_detail[$ii]['Discount'] = $Discount_Detail->Discount_Percent;
        $array_dicount_detail[$ii]['Amount'] = $Discount_Detail->Discount_Amount;
        $ii++;
    }
    // echo json_encode($array_dicount_detail);
   
    $array_dicount_member = [];
    $a=0;
    foreach ($Cost_Scheme_Discount_Member as $Discount_Member) {
        $array_dicount_member[$a]['Register_Number'] = $Discount_Member->Register_Number;
        $array_dicount_member[$a]['NimName'] = $Discount_Member->Register_Number.' | '.$Discount_Member->Full_Name;
        $a++;
    }

    // echo json_encode($array_dicount_member);
    ?>
    ...
    <script type="text/javascript">   
        ...
        $(document).ready(function () {
            ...
            var array_detail = <?php echo json_encode($array_dicount_detail); ?>;
            
            for(var i =0; i<array_detail.length;i++){
                DiscountSchemeDetail[i] = array_detail[i];
            }
            //generate array_detail  to table view
            ...
            var Mahasiswa = [];
            var array_member = <?php echo json_encode($array_dicount_member); ?>;
            // console.log(array_member);
            for(var i =0; i<array_member.length;i++){
                Mahasiswa[i] = array_member[i];
            }
            ...
            $('#submit').click(function () {
                ...
                var Cost_Scheme_Discount_Id = $("#Cost_Scheme_Discount_Id").val();
                ...
                if (isAllValid) {
                    ...
                    $.ajax({
                        ...
                        type: "POST",
                        url: "{{ url('diskon_biaya/edit_post') }}",
                        beforeSend : function() {
            ...
            function GeneratedItemsTable() {
                if (DiscountSchemeDetail.length > 0) {
                    var $table = $('<table id="DiscountTable" class="table table-striped table-bordered table-hover table-sm table-font-sm">');
            ...
            function GeneratedItemsTable2() {
                    if (Mahasiswa.length > 0) {
                        var $table = $('<table id="MahasiswaTable" class="table table-striped table-bordered table-hover table-sm table-font-sm">');
